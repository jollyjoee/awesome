...
WinHttp.Open("GET", "https://worldtimeapi.org/api/timezone/asia/manila.txt", false)
WinHttp.Send()
...
Else
{
	...
	MsgBox, 16, Application, An error has occured in retrieving current file version.`nAuto-update may not work until this issue is resolved.`nPress "Ok" to ignore this message and continue to the script.
	Sleep, 500
	goto, LicenseCheck
}
Return
...
Notreg:
MsgBox, you are not registered yet. Click OK and send your license to the developer.
Clipboard := keyencrypted
ExitApp
...
try{
   ...
   pos := InStr(dbkey, keyencrypted) ;checks if %keyencrypted% was found (1) or not (0) in %dbkey%
   If (pos > 0) ;now checks for the expiration date, if it is still valid
   {
